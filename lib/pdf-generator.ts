...
export const generateQuotationPdf = async (data: QuotationData) => {
  ...
  const MAX_Y = 270; // Limite inferior seguro para conteúdo antes do rodapé (282mm)
  let y = (doc as any).lastAutoTable.finalY + 12;

  // 1. Verificar se o bloco de condições cabe na página atual
  if (y + 20 > MAX_Y) {
    doc.addPage();
    y = 20;
  }
  ...
  // 2. Tratar bloco de Observações e Assinatura
  const observacaoTexto = data.obs.trim();
  let fontSizeObs = 9.5;
  let lineSpacingObs = 3.8;
  let linhasObs: string[] = [];

  if (observacaoTexto) {
    doc.setFontSize(fontSizeObs);
    doc.setFont("helvetica", "normal");
    linhasObs = doc.splitTextToSize(observacaoTexto.toUpperCase(), 174);
    
    // Se o texto for longo, reduz o tamanho da fonte e espaçamento
    if (linhasObs.length > 12) {
      fontSizeObs = 8;
      lineSpacingObs = 3.2;
      doc.setFontSize(fontSizeObs);
      linhasObs = doc.splitTextToSize(observacaoTexto.toUpperCase(), 174);
    } else if (linhasObs.length > 6) {
      fontSizeObs = 8.5;
      lineSpacingObs = 3.5;
      doc.setFontSize(fontSizeObs);
      linhasObs = doc.splitTextToSize(observacaoTexto.toUpperCase(), 174);
  ...
  const textContentHeight = linhasObs.length > 0 ? ((linhasObs.length - 1) * lineSpacingObs + 4) : 0;
  const calculatedBoxHeight = observacaoTexto ? (10 + textContentHeight + 4) : minBoxHeight;
  let boxHeight = Math.max(minBoxHeight, calculatedBoxHeight);

  const sigBlockHeight = 25; // Espaço para gap + linha de assinatura + texto
  const neededSpace = boxHeight + sigBlockHeight;

  // Se o espaço necessário para Observações + Assinatura ultrapassar o limite da página atual
  if (y + neededSpace > MAX_Y) {
    // Se não estivermos já no topo de uma nova página, move para uma nova página
    if (y > 35) {
      doc.addPage();
      y = 20;
    }
    
    // Garante que a altura da caixa não empurre a assinatura para fora da área segura
    const maxBoxHeight = MAX_Y - y - sigBlockHeight;
    if (boxHeight > maxBoxHeight) {
      boxHeight = Math.max(minBoxHeight, maxBoxHeight);
    }
  }

  // Renderizar a caixa de Observações
  doc.setFillColor(232, 237, 247);
  ...
  if (observacaoTexto && linhasObs.length > 0) {
    doc.setFontSize(fontSizeObs);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(0);
    
    // Limitar o número de linhas impressas ao tamanho da caixa
    const maxLinesThatFit = Math.max(1, Math.floor((boxHeight - 11) / lineSpacingObs));
    const visibleLines = linhasObs.slice(0, maxLinesThatFit);
    
    doc.text(visibleLines, 18, y + 11);
  }

  // Renderizar a Assinatura
  let ySig = y + boxHeight + 12;
  doc.setTextColor(0);
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  ...
  // --- RODAPÉ AZUL (Aplicado em todas as páginas) ---
